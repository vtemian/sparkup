...
How to run it, and every trap in it: the skills under `.claude/skills/` and the README's
install flow. This file is only the rules.
...
agent would otherwise undo; record the *evidence* beside the rule it settled, not the chronology.
...
text holds what and why. Repo facts belong here, procedures in the skills under
`.claude/skills/`.
